        ...
        "components": {
            "schemas": {
                ...
                "TestResultsRequest": {
                    ...
                    "properties": {
                        "customerId": {"type": "integer", "description": "Customer identifier"},
                        "sourceSystem": {"type": "string", "description": "Source system (e.g., ROBOT, TESTING_ENV)", "default": "ROBOT"},
                        "events": {
                            "type": "array",
                            "items": {
                                "oneOf": [
...
def process_test_results():
    """Comprehensive Test Results API - handles all types of test data"""
    try:
        data = request.get_json()
        if not data:
            return jsonify({"error": "No data provided"}), 400
        ...
        source_system = data.get('sourceSystem', 'ROBOT')
        events = data.get('events', [])
        ...
        for event in events:
            try:
                ...
                if event_kind == 'TEST_RUN':
                    result = process_test_run_event(event, customer_id, source_system)
